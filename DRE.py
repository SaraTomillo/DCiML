...
class DensityRatioEstimator:
        ...
        if len(self.sigmas) * len(self.num_params) > 1:
            ...
            for num_param in self.num_params:
                for sigma in self.sigmas:
                    ...
                    # JR Quevedo: iteritems es Python 2
    ...
    def _phi(self, X, sigma=None):
        ...
        if self._phi_fitted:
            ...
            # Star QUEVEDO Equivalent code without extra memory allocation
            # Only memory for final result
            test_vectors = self._test_vectors;
            finRows = len(X)
            finCols = len(test_vectors)
            vLen = len(test_vectors[0])  # same as len(X[0][0]])
            divi = (2 * sigma ** 2)
            finResult = np.zeros([finRows, finCols])  # Unique memory allocation in algorithm
            for iRow in range(finRows):
                for iCol in range(finCols):
                    # X[i]-test_vectors[i]
                    vsum = 0
                    for i in range(vLen):
                        vsum = vsum + (X[iRow][0][i] - test_vectors[iCol][i]) ** 2
                    finResult[iRow][iCol] = np.exp(-vsum / divi)
            return finResult
            # end Equivalent code QUEVEDO
